 ...
 * @author xEdushx
 */
public class mon_transacciones extends sis_pantalla {

    private final static Logger LOGGER = Logger.getLogger(mon_transacciones.class.getName());
...
//        pf_grid gri_cabecera = new pf_grid();
    ...
    public void filtrar() {
        ...
        if (ldat_fecha_inicial != null && ldat_fecha_inicial != null) {
            ...
            if (!istr_id_adm_formulario.isEmpty() && !istr_id_adm_formulario.equalsIgnoreCase("null") && !istr_id_usuario.isEmpty() && !istr_id_usuario.equalsIgnoreCase("null")) {
                lstr_condicion = "vc.vis_fecha_registro >='" + lstr_fecha_inicial + " 00:00:00' "
                        + "and vc.vis_fecha_registro <='" + lstr_fecha_final + " 23:59:59' "
                ...
                lstr_condicion = "vc.vis_fecha_registro >='" + lstr_fecha_inicial + " 00:00:00' "
                        + "and vc.vis_fecha_registro <='" + lstr_fecha_final + " 23:59:59' and ";
            }
    ...
    public void configurar_mapa(String astr_condicion) {
        ...
        tbl_puntos_gps.setSql("SELECT vc.vis_codigo as ID, "
                + "em.nombre as EMPRESA, "
                + "cl.cli_nombre as CLIENTE, "
                + "us.nombre_completo as USUARIO, "
                + "vc.vis_fecha_registro as FECHA, "
                + "vc.vis_latitud as LATITUD, "
                + "vc.vis_longitud as LONGITUD, "
                + "vc.vis_imei as IMEI "
                + "FROM mp_vc_visita vc, "
                + "tbl_empresa em, "
                + "mp_vc_clientes cl, "
                + "tbl_usuario us "
                + "where em.id_empresa = " + sis_soporte.obtener_instancia_soporte().obtener_empresa() + " and "
                + astr_condicion
                + "em.id_empresa = vc.id_empresa and "
                + "cl.cli_codigo = vc.cli_codigo and "
                + "us.id_usuario = vc.id_usuario and "
...
//            pan_datos.getChildren().add(new pf_etiqueta("ID:"));
//            pan_datos.getChildren().add(new pf_etiqueta(tbl_puntos_gps.getValor(aint, "ID")));
//
//            pan_datos.getChildren().add(new pf_etiqueta("IMEI:"));
//            pan_datos.getChildren().add(new pf_etiqueta(tbl_puntos_gps.getValor(aint, "IMEI")));
